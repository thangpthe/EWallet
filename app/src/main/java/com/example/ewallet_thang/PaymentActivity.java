...
public class PaymentActivity extends AppCompatActivity {
    ...
    private static final int REQUEST_VOUCHER     = 3001;
    private static final int REQUEST_VISA_CARD   = 3002;
    ...
    // ── NEW: payment source views ─────────────────────────────────
    private CardView     tabPayWallet, tabPayVisa;
    private TextView     tvTabPayWallet, tvTabPayVisa;
    private LinearLayout layoutVisaInfo;
    private TextView     tvSelectedCard;
    private Button       btnChangeCard, btnManageCards;
    ...
    // Visa state
    private boolean useVisa      = false;   // false = Ví điện tử, true = Thẻ Visa
    private String  visaCardId   = null;
    private String  visaLast4    = null;
    private String  visaHolder   = null;
    private String  visaExpiry   = null;
    private String  visaBrand    = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupListeners();
        selectTab(0);
        selectPaymentSource(false);   // default = Ví điện tử
    }
    ...
    private void initViews() {
        ...
        // NEW – payment source
        tabPayWallet   = findViewById(R.id.tabPayWallet);
        tabPayVisa     = findViewById(R.id.tabPayVisa);
        tvTabPayWallet = findViewById(R.id.tvTabPayWallet);
        tvTabPayVisa   = findViewById(R.id.tvTabPayVisa);
        layoutVisaInfo = findViewById(R.id.layoutVisaInfo);
        tvSelectedCard = findViewById(R.id.tvSelectedCard);
        btnChangeCard  = findViewById(R.id.btnChangeCard);
        btnManageCards = findViewById(R.id.btnManageCards);
    ...
    private void setupListeners() {
        ...
        // NEW – payment source tabs
        tabPayWallet.setOnClickListener(v -> selectPaymentSource(false));
        tabPayVisa.setOnClickListener(v   -> selectPaymentSource(true));

        btnChangeCard.setOnClickListener(v  -> openVisaPicker());
    ...
    /**
     * @param visa true = Thẻ Visa/Mastercard, false = Ví điện tử
     */
    private void selectPaymentSource(boolean visa) {
        useVisa = visa;

        if (tabPayWallet != null) {
            tabPayWallet.setCardBackgroundColor(visa ? 0xFFEEF0FF : 0xFF3D5AFE);
            tvTabPayWallet.setTextColor(visa ? 0xFF5063C8 : 0xFFFFFFFF);
            tabPayVisa.setCardBackgroundColor(visa ? 0xFF3D5AFE : 0xFFEEF0FF);
            tvTabPayVisa.setTextColor(visa ? 0xFFFFFFFF : 0xFF5063C8);
        }

        if (layoutVisaInfo != null) {
            layoutVisaInfo.setVisibility(visa ? View.VISIBLE : View.GONE);
        }

        if (visa && visaLast4 == null) {
            // Chưa có thẻ -> mở picker ngay
            openVisaPicker();
        }
    ...
    // ─── openVisaPicker (NEW) ─────────────────────────────────────
    private void openVisaPicker() {
        Intent i = new Intent(this, VisaCardActivity.class);
        i.putExtra(VisaCardActivity.EXTRA_PICK_MODE, true);
        startActivityForResult(i, REQUEST_VISA_CARD);
    }
    ...
    private void updateSummary() {
        ...
        String payLabel = useVisa && visaLast4 != null
                ? "Thanh toán bằng " + visaBrand + " *" + visaLast4
                : "Thanh toán";
        btnPay.setText(amount > 0
                ? payLabel + "  " + currencyFormat.format(finalAmount) + "đ"
                : payLabel);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        // Visa card result (NEW)
        if (requestCode == REQUEST_VISA_CARD) {
            if (resultCode == Activity.RESULT_OK && data != null) {
                visaCardId  = data.getStringExtra("cardId");
                visaLast4   = data.getStringExtra("last4");
                visaHolder  = data.getStringExtra("cardHolder");
                visaExpiry  = data.getStringExtra("expiry");
                visaBrand   = data.getStringExtra("cardBrand");

                if (tvSelectedCard != null) {
                    tvSelectedCard.setText(
                            visaBrand + "  ****  " + visaLast4 + "\n" + visaHolder + "   " + visaExpiry);
                }
                updateSummary();
            } else {
                // User cancelled and no card was previously selected
                if (visaLast4 == null) {
                    selectPaymentSource(false);  // fall back to wallet
                }
            }
        }
    }
    ...
    private void processPayment() {
        ...
        // Kiểm tra thẻ Visa đã chọn chưa
        if (useVisa && visaLast4 == null) {
            Toast.makeText(this, "Vui lòng chọn thẻ Visa để thanh toán", Toast.LENGTH_SHORT).show();
            openVisaPicker();
            return;
        }
        ...
        if (!useVisa && deduct > currentBalance) {
            Toast.makeText(this, "Số dư không đủ! Cần " + currencyFormat.format(deduct) + "đ",
                    Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        // Chỉ trừ ví nếu thanh toán bằng ví
        if (!useVisa) {
            batch.update(db.collection("users").document(userPhone),
                    "balance", FieldValue.increment(-deduct));
        }
        ...
        tx.put("date",           date);
        tx.put("timestamp",      System.currentTimeMillis());
        // NEW: ghi nhận phương thức thanh toán
        tx.put("paymentMethod",  useVisa ? "VISA" : "WALLET");
        if (useVisa) {
            tx.put("visaLast4",   visaLast4);
            tx.put("visaBrand",   visaBrand);
            tx.put("visaHolder",  visaHolder);
        }
        batch.set(db.collection("transactions").document(), tx);
